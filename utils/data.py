...
# Wrapper to create Single-View datasets starting from 1 view and augmentation
class MultiViewDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.apply_same:
            ...
            v_2 = F.to_tensor(x)   # 原来是 self.augmentation(x)
        ...
        #print(v_1.shape)
        #print(v_2.shape)
        c = torch.cat((v_1, v_2), axis=0)
        col_idxs = list(range(c.shape[2]))
        #print(c.shape)
        random.shuffle(col_idxs)
        c = c[:, :,  torch.tensor(col_idxs)]
        return v_1, v_2, c,y
