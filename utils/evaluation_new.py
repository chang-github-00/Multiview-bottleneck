...
class EmbeddedDataset:
    ...
    def _embed(self, encoder, dataset, device):
        ...
        reps = []
        with torch.no_grad():  
            for _,_,x, y in data_loader:
                x = x.to(device)
                y = y.to(device)

                p_z_given_x = encoder(x)  #distribution of z|x when do evaluation,use mean of distribution z|x

                reps.append(p_z_given_x.mean.detach())       #reps.append(p_z_given_x.mean.detach()) 


                ys.append(y)

            ys = torch.cat(ys, 0)       #concat along row ([x],[x],[x]) (x,x,x) 
...
def split(dataset, size, split_type):
    if split_type == 'Random':
        ...
        class_ids = {}
        for idx, (_,_,_, y) in enumerate(dataset):
